...
unconfigured or any failure (bad token, actor error, timeout) returns []
rather than raising, so one broken tier can't take down the rest of the crawl.
"""
...
def _run_actor(actor, payload):
    ...
    try:
        ...
        # APIFY_TWITTER_SEARCH_ACTOR) must have its slash swapped for "~", or
...
def apify_twitter_search_posts(query, source_url, source_name):
    ...
    across all of X, via Apify's hosted tweet-search actor."""
    tweets = _run_actor(
