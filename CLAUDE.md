...
- **Database-local identifiers must not be exported.** `pipeline_run_id` is a foreign key into *this* database's `pipeline_runs`; exported, every row fails `articles_pipeline_run_id_fkey` on the importing side and nothing lands (`EXPORT_LOCAL_ONLY_FIELDS`). `story_id` is excluded for the same reason - it names a local `story_groups` row, and the importing side regroups by body similarity itself.
